...
#include "RFM69/RFM69registers.h"
  ...
  #define WIRINGPISPI_SPEED 500000
#else
  #include "SPI/SPI.h"
#endif
...
RFM69::RFM69(uint8_t slaveSelect, uint8_t interrupt, bool isRFM69HW)
{
...
{
#if defined (RASPBERRY) || defined (RF69_ATTACHINTERRUPT_TAKES_PIN_NUMBER)
  _interruptNum = _interrupt;
#else
  _interruptNum = digitalPinToInterrupt(_interrupt);
#endif
...
#ifdef RASPBERRY
  if( wiringPiSPISetup(_slaveSelect, WIRINGPISPI_SPEED) < 0 )
  {
...
void RFM69::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK)
{
...
#ifdef RASPBERRY
  uint8_t thedata[RF69_FIFO_SIZE];

  thedata[0] = _WRITE_ACCESS(REG_FIFO);
  ...
  {
    thedata[i + 5] = ((uint8_t*)buffer)[i];
  }
  // write to FIFO
  wiringPiSPIDataRW( _slaveSelect, thedata, bufferSize + 5 );
#else
  // write to FIFO
  select();

  SPI.transfer( _WRITE_ACCESS(REG_FIFO) );
  ...
  while (digitalRead(_interrupt) == 0 && millis() - txStart < RF69_TX_LIMIT_MS); // wait for DIO0 to turn HIGH signalling transmission finish
...
void RFM69::interruptHandler() {
  if( (_mode == RF69_MODE_RX) && ((readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY) == RF_IRQFLAGS2_PAYLOADREADY) )
  {
    uint8_t i;
    ...
    thedata[0] = _READ_ACCESS(REG_FIFO);
    thedata[1] = 0; // PAYLOADLEN
    thedata[2] = 0; //  TargetID
    wiringPiSPIDataRW( _slaveSelect, thedata, 3 );
    delayMicroseconds( MICROSLEEP_LENGTH );

    PAYLOADLEN = (thedata[1] > RF69_FIFO_SIZE) ? RF69_FIFO_SIZE : thedata[1]; // precaution
    TARGETID   = thedata[2];
#else
    select();

    SPI.transfer( _READ_ACCESS(REG_FIFO) );
    PAYLOADLEN = SPI.transfer( 0 );
    PAYLOADLEN = (PAYLOADLEN > RF69_FIFO_SIZE) ? RF69_FIFO_SIZE : PAYLOADLEN; // precaution
    ...
    thedata[2] = 0; //CTLbyte;
    wiringPiSPIDataRW( _slaveSelect, thedata, DATALEN + 3 );

    SENDERID = thedata[1];

    uint8_t CTLbyte = thedata[2];
    ACK_RECEIVED    = CTLbyte & RFM69_CTL_SENDACK; //extract ACK-requested flag
    ACK_REQUESTED   = CTLbyte & RFM69_CTL_REQACK; //extract ACK-received flag
...
void RFM69::encrypt( const char* key )
{
...
#ifdef RASPBERRY
  unsigned char thedata[RF69_ENCRYPTION_KEY_SIZE + 1];

  if( key != nullptr )
  {
    thedata[0] = _WRITE_ACCESS(REG_AESKEY1);
    memcpy( &(thedata[1]), key, RF69_ENCRYPTION_KEY_SIZE );
    wiringPiSPIDataRW( _slaveSelect, thedata, (RF69_ENCRYPTION_KEY_SIZE + 1) );
    delayMicroseconds( MICROSLEEP_LENGTH );
  }
#else
    if( key != nullptr )
  ...
  #if defined(RF69_LISTENMODE_ENABLE)
      memcpy(_encryptKey, key, RF69_ENCRYPTION_KEY_SIZE);
  #endif

      select();
      SPI.transfer( _WRITE_ACCESS(REG_AESKEY1) );
      for( uint8_t i=0; i<RF69_ENCRYPTION_KEY_SIZE; i++ )
      {
        SPI.transfer( key[i] );
      }
      unselect();
    ...
    writeReg( REG_PACKETCONFIG2, (readReg(REG_PACKETCONFIG2) & ~RF_PACKET2_AES_ON) | ((key != nullptr) ? RF_PACKET2_AES_ON : RF_PACKET2_AES_OFF) );
}
...
#ifdef RASPBERRY
  uint8_t thedata[2];
  thedata[0] = _READ_ACCESS(addr);
  thedata[1] = 0;

  wiringPiSPIDataRW( _slaveSelect, thedata, 2 );
  delayMicroseconds( MICROSLEEP_LENGTH );
...
#else
  select();
  SPI.transfer( _READ_ACCESS(addr) );
...
#else
  select();
  SPI.transfer( _WRITE_ACCESS(addr) );
  SPI.transfer( value );
  unselect();
#endif
}
...
{
  _slaveSelect = newSPISlaveSelect;
...
#if defined (ARDUINO) && defined (READ_ALL_REGS)
